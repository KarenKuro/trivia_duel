...
export class FirstMigration1722326497877 implements MigrationInterface {
  name = 'FirstMigration1722326497877';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`users\` DROP COLUMN \`premiumCoins\``,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`users\` ADD \`status\` enum ('ACTIVE', 'LOCKED') NOT NULL DEFAULT 'ACTIVE'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`users\` ADD \`coins\` int NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`users\` ADD \`premiumCoins\` int NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`users\` ADD \`subscription\` tinyint NOT NULL DEFAULT 0`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`users\` ADD \`lives\` int NOT NULL DEFAULT '5'`,
    );
    await queryRunner.query(
